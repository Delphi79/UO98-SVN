...
#include "pipes.cpp"
#include "process.cpp"
#include "startup_info.cpp"
#include "security_attributes.cpp"

static InterfaceToUOSL InterfaceToUOSL;
...
unsigned char GetCurrentScriptType()
{
  ...
  return _AL; // 0 = Unknown, 1 = M, 2 = non-verified UOSL, 3 = verified UOSL
}
...
unsigned int GetCurrentScriptExtraData()
{
  __asm
  {
    mov edx, 0x63E128    // Points to the current script being compiled (Being-CompiledObject class)
    mov ecx, [edx]
    mov eax, [ecx + 0x55] // See MyCompiledObjectConstructor...
  }
  return _EAX;
}

void SetCurrentScriptExtraData(unsigned int ExtraData)
{
  __asm
  {
    mov eax, ExtraData
    mov edx, 0x63E128    // Points to the current script being compiled (Being-CompiledObject class)
    mov ecx, [edx]
    mov [ecx + 0x55], eax // See MyCompiledObjectConstructor...
  }
}

/*********************************************/
/*                                           */
/* FUNCTIONS USED BY THE JIT-TOKEN CONVERTOR */
/*                                           */
/*********************************************/
...
typedef unsigned int (* __cdecl __OpenFileByCode)(unsigned int DirectoryCode, char *ScriptNameWithM, const char *OpenMode);
__OpenFileByCode OpenFileByCode = (__OpenFileByCode) 0x480877;
...
int __declspec(naked) MyCompiledObjectConstructor()
{
  __asm
  {
     ...
     mov byte  ptr [ecx + 0x54], 0 // Is verified UOSL (3), non-verified UOSL (2) or M (1) or Unknown (0) file ?
     mov dword ptr [ecx + 0x55], 0 // A free to use extra byte, currently unused
     mov eax, 0x426E20
     jmp eax
  }
...
void HackTheStack(unsigned int return_address_to_detect, void *should_be, void *replace_by)
{
  void **Stack = (void **) _ESP;
  while(true)
  {
    if((unsigned int) *Stack -- != return_address_to_detect)
	  if(*Stack == should_be)
		break;
  }
  *Stack = replace_by;
...
#define USE_ORIGINAL_ALGORITHM NULL

char * __stdcall MyGetRawScriptToken(char *Walker, char *Target)
{
  if(!*Walker)
    return USE_ORIGINAL_ALGORITHM;
  if(GetCurrentScriptType() == 0)
  {
    if(IsObfuscatedScriptByte(* (unsigned short *) Walker))
      SetCurrentScriptType(1);
	else if(!InterfaceToUOSL.IsActive())
      SetCurrentScriptType(2);
	else
	{
      SetCurrentScriptType(3);
 
	  char *Result = InterfaceToUOSL.TranslateScript(Walker);	  
	  if((!Result) || (int(Result) & 0x80000000))
	  {
		char *ErrorMessage = (char *)(int(Result) & 0x7FFFFFFF);
		MessageBox(HWND_DESKTOP, Result != NULL ? ErrorMessage : "Unknown error!", "JIT/UOSL", MB_OK);
	    *Walker = '\0';
		return USE_ORIGINAL_ALGORITHM;
	  }

	  SetCurrentScriptExtraData((unsigned int) Result);
	  HackTheStack(0x427015, Walker, Result);
	  Walker = Result;
	}
  }
...
unsigned int __cdecl MyOpenScriptFile(const unsigned int DirectoryCode, char *ScriptNameWithM, const char * const OpenMode)
{
  unsigned int RetCode;

  // NOTE: do not modify the DirectoryCode and/or OpenMode
  RetCode = OpenFileByCode(DirectoryCode, ScriptNameWithM, OpenMode);
  if(RetCode)
  {
    printf("%s -> M-file\n", ScriptNameWithM);
    return RetCode;
  ...
  RetCode = OpenFileByCode(DirectoryCode, NewFileName, OpenMode);
  if(RetCode)
  {
    printf("%s -> UOSL-file (native or enhanced without extensions!)\n", NewFileName);
    return RetCode;
  }
...
BOOL WINAPI DllEntryPoint(HINSTANCE hInstance, ULONG ulReason, LPVOID)
{
  ...
  if(ulReason == DLL_PROCESS_ATTACH)
  {
    ...
    puts("Welcome to the UO Demo JIT Console!");

    // Prepare the patches
    SetRel32_AtPatch(&PI_Compiler_CompiledObjectConstructor, MyCompiledObjectConstructor);
    SetRel32_AtPatch(&PI_Intercept_LoadScript, MyOpenScriptFile);
